...
const Teachers = ({ user }) => {
  ...
  // Fetch faculty/teachers on component mount
  useEffect(() => {
    fetchTeachers();
  }, [user]);
  ...
  const stats = {
    available: teachers.filter((t) => t.status === "available").length,
    busy: teachers.filter((t) => t.status === "busy").length,
    totalAppointments: teachers.reduce(
      (sum, t) => sum + t.appointments.length,
      0
    ),
    totalClients: teachers.reduce((sum, t) => sum + t.totalClients, 0),
  };
  ...
  const filteredTeachers = teachers
    .filter((teacher) => {
      ...
      if (activeTab === "available" && teacher.status !== "available")
        return false;
      if (activeTab === "busy" && teacher.status !== "busy") return false;

      if (activeFilter === "mainBranch" && teacher.branch !== "Main Branch")
        return false;
      if (activeFilter === "northBranch" && teacher.branch !== "North Branch")
        return false;
    ...
    <TeachersContainer>
            ...
            >
              ...
              <div className="detail-section">
                ...
                <div className="appointment-list">
                  {selectedTeacher.appointments.length > 0 ? (
                    selectedTeacher.appointments.map((appointment) => (
                      <div className="appointment-item" key={appointment.id}>
                        <div className="appointment-info">
                          <div className="appointment-title">
                            {appointment.service} with {appointment.client}
                          </div>
      ...
      {showScheduleModal && selectedTeacher && (
        ...
        >
          ...
          >
            <h2>
              <FaCalendarAlt /> Add Availability
            </h2>

            <div className="form-group">
              <label>Teacher</label>
              <input type="text" value={selectedTeacher.name} disabled />
            </div>
            ...
            <div className="modal-actions">
              ...
              <Button
                ...
                style={{
                  opacity:
                    !newAvailability.day || !newAvailability.time ? 0.7 : 1,
                }}
              >
                <FaPaperPlane /> Add
              </Button>
